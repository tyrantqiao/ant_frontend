...
@connect(({ values, loading }) => ({
  // 提交中，根据loading.effects对象判断异步请求是否完成
  submitting: loading.effects['values/submitStepForm'],
  data: values.step,
}))
@Form.create()
    ...
    const onValidateForm = e => {
      ...
      validateFields((err, values) => {
        if (!err) {
          ...
          // 所以这里val/submitStepForm就是Models/values.js中定义的，val是namespace名字
          dispatch({
            // type不可缺省，其他payload、error、meta等可缺省
            type: 'values/submitStepForm',
