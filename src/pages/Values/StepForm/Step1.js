...
import { Form, DatePicker, TimePicker, Input, Button, Select, Divider, Checkbox } from 'antd';
...
// val.step把val.setp赋值给data 被connect的组件自动有dispatch函数
@connect(({ values }) => ({ data: values.step, nodes: values.nodes }))
// 这样包装后的组件会自带 this.props.form 属性
@Form.create()
class Step1 extends React.PureComponent {
  getSelectNodes = () => {
    this.props.dispatch({ type: 'values/getNodes' });
  };
  render() {
    // form是表单的对象，下面两个属性为form的属性，与表单绑定以及字段校验
    const { form, dispatch, data } = this.props;
    const { getFieldDecorator, validateFields } = form;
    let nodeOptions = this.props.nodes.map(node => <Option key={node.id}>{node.node_name}</Option>);
    const onValidateForm = () => {
      // 获取表单数据并对values进行校验，若values未设置则对全部内容进行校验
    ...
    return (
      <Fragment>
        <Form layout="horizontal" className={styles.stepForm} hideRequiredMark>
          ...
          <Form.Item {...formItemLayout} label="数据节点id">
            {getFieldDecorator('nodeId', {
              ...
              <Select combobox onDropdownVisibleChange={this.getSelectNodes}>
                {nodeOptions}
              </Select>
            )}
          </Form.Item>
        ...
        <div className={styles.desc}>
          <h3>表单提示</h3>
          <h4>为数据节点添加数据：</h4>
          <p>主要是以传感器的型号来指定的，然后添加数据即可。</p>
          <h4>数值表的导入</h4>
          <p>手动导入时应注意选择对应的传感器id，然后导入数据需要选择好时间单位等等。</p>
        </div>
      </Fragment>
    );
  }
