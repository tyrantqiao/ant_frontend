...
import { Form, Input, Button, Select, Table, Divider, Tag } from 'antd';
...
class ListForm extends React.PureComponent {
  ...
  list = [
    ...
    },
    {
      ...
      render: node_type => (
        <span>
          <Tag color={node_type.length > 5 ? 'geekblue' : 'green'} key={node_type}>
    ...
    },
    {
      ...
      render: (text, record) => (
        <span>
          ...
          <a href="javascript:;" onClick={() => this.deleteNode({ id: record.id })}>
            Delete
          </a>
        </span>
      ),
  ...
  // 更新node，更新完后刷新数据列表
  updateNode = params => {
    this.props.dispatch({ type: 'node/updateNode', payload: params });
    this.fetch();
  };

  // 删除node，更新完后刷新数据列表
  deleteNode = id => {
    this.props.dispatch({ type: 'node/deleteNode', payload: id });
  ...
  getNodes = () => {
    this.props.dispatch({ type: 'node/getNodesList' });
  };
    ...
    return (
      <Fragment>
        <Form layout="horizontal" hideRequiredMark>
          {/* 展示数据 */}
          <Table
            columns={this.list} // react规范要求rowKey需要选择唯一字段作为key，或者自建一个key出来
            // record则是每一行的记录
